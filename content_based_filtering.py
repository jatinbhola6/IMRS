...
class CB_Filtering():
	def converting_to_dummy_variable(file_name = 'test.csv',movie_features=['_id','director','creator','actor_list','genre'],out_file='dummy_data_for_clustering.csv'):
		...
		movie_data[movie_features] = movie_data[movie_features].applymap(lambda x:ast.literal_eval(x))
	...
	def similarity_matrix(file_name='dummy_data_for_clustering.csv',def_index='_id',sim_file='similarity_matrix.npy'):
	...
	def get_recommendations(movie_id,sim_file='similarity_matrix.npy',movie_file='test.csv',no_recom=5):
		...
		movie_index = movie_list.index.get_loc(movie_id)
		top_recommendations = np.argsort(cos_sim[movie_index])[::-1][1:6]
		recom_ids = [movie_list.iloc[i]['_id'] for i in top_recommendations]
		return recom_ids

	def add_new_movie(movie_id,movie_file='test.csv',movie_features=['_id','director','creator','actor_list','genre'],dummy_file = 'dummy_data_for_clustering.csv'):
